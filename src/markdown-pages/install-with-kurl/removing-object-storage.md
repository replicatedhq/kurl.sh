...
> **Important**: Velero 1.17.0 and later uses the Kopia uploader and no longer supports the [Local-Volume-Provider](https://github.com/replicatedhq/local-volume-provider) plugin. On Velero 1.17 and later, Velero backups require an S3-compatible object store. The **Internal Storage** destination is satisfied by the in-cluster MinIO or Rook object store, and the **Host Path** and **Network File System (NFS)** destinations are backed by a KOTS-deployed MinIO instance (`kotsadm-fs-minio`) that stores data on the host path or NFS share — both work on Velero 1.17 and later when the MinIO add-on is installed and kotsadm is 1.131.6 or later. However, removing object storage from the cluster is not supported on Velero 1.17 and later, because the `kotsadm.disableS3` flag and older versions of KOTS configure Velero with the Local Volume Provider plugin. Clusters already configured with a Local Volume Provider destination must move Velero back onto an object store to upgrade; see [Upgrading to Velero 1.17 or Later](/docs/install-with-kurl/removing-object-storage#upgrading-to-velero-1-17-or-later).
...
1. **Velero**:
   ...
   - On Velero 1.17 and later, the Local Volume Provider plugin is not supported, but the Host Path and NFS destinations still work when the MinIO add-on is installed (kotsadm 1.131.6 or later): KOTS backs them with a KOTS-deployed MinIO instance (`kotsadm-fs-minio`) that stores data on the host path or NFS share, and Velero uses a standard S3 BackupStorageLocation. Installing or upgrading with `kotsadm.disableS3: true` stops the installation with an error, because that flag configures Velero with the Local Volume Provider. Add the MinIO or Rook add-on, configure an external S3-compatible object store, or use a Velero version earlier than 1.17.
...
Velero 1.17.0 replaced the restic uploader with Kopia. Kopia cannot read existing restic repositories and does not support the [Local-Volume-Provider](https://github.com/replicatedhq/local-volume-provider) plugin, so a cluster whose default BackupStorageLocation uses a Local Volume Provider (`replicated.com/hostpath`, `replicated.com/nfs`, or `replicated.com/pvc`) must move Velero back onto an object store to upgrade. Clusters with the MinIO add-on installed and kotsadm 1.131.6 or later are not affected: their Host Path and NFS destinations use a KOTS-deployed MinIO instance instead of the Local Volume Provider.

When you upgrade a cluster that is configured with a Local Volume Provider destination to Velero 1.17 or later:
